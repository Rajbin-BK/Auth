...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($userData) {
            // User exists, verify the password using sha256 and stored salt
            $sha256HashAttempt = hash('sha256', $password . $userData['salt']);

            if (password_verify($sha256HashAttempt, $userData['password_sha256'])) {
                // Successful login
                $_SESSION['user'] = [
                    'email' => $email,
    ...
    <div class="mt-8">
        ...
        <form method="post" action="login-form-bcrypt.php" autoComplete="off">
                ...
                <div class="flex flex-col mb-6">
                    ...
                    <?php
                        if (isset($loginError) && $loginError) {
                            echo '<div class="flex ml-auto text-red-600">Invalid email or password</div>';
                        }
                    ?>
                    </div>
            ...
            <div class="flex items-center justify-center mt-6">
                <a href="registration-form-salt.php" class="inline-flex items-center text-xs font-thin text-center text-gray-500 hover:text-gray-700 dark:text-gray-100 dark:hover:text-white">
                    <span class="ml-2">
